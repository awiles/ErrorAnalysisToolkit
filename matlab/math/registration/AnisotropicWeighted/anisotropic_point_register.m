function [R,t,FRE,n,isConverged] = anisotropic_point_register(X,Y,W,threshold)
...
% December 2008
if nargin<2
    error('X and Y must be given as input');
end
if size(X,1)~=3 && size(Y,1)~=3
    error('X and Y must be 3 by N.');
end
N = size(X,2);
if size(Y,2)~=N
