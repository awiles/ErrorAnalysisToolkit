...
theoryRMSAvgFLE = 11;
RMSDiffAvgFLE = 12;
percentDiffAvgFLE = 13;

load parm;
figure(1);
plot(data(:,theoryRMS), data(:,measRMS), '.k');
hold on;
plot(data(:,theoryRMSAvgFLE), data(:,measRMSAvgFLE), '.r');
...
subtitlestring = sprintf('%s FLE Model, %d Fiducials',...
    fleModel, parm.nMarkers);
...
for i = 1:5 % only plot the histograms for the first 5 tests.
    ...
    hold off;
    xlim([0, max(histbins)]);
    legend('Monte Carlo', 'Measured TRE', 'NH TRE', 'Avg FLE TRE');
    titlestring = sprintf('TRE Distribution %06d', i);
    subtitlestring = sprintf('Meas. RMS: %3.2f, NH RMS: %3.2f, Avg FLE RMS: %3.2f',...
        testResult.probe.RMS.Meas, testResult.probe.RMS.Theory,...
        testResult.probeAvgFLE.RMS.Theory);
